...
model_a <- glm(delayed_fn ~ log(glutwa) + d_age + as.factor(d_cod) + hemo_instability , data = df, family = "binomial")
...
# Calculate the standard error of relative_doubling_odds_ratio
std_err <- sqrt(var)
...
coef_logTWA_a <- coef(model_a)["log(glutwa)"]
exp(coef_logTWA_a*log(2))
